...
with tab_user:
    ...
    with st.form("ticket_form", clear_on_submit=True):
        ...
        issue_desc = st.text_area(t[lang]["desc"]) # هذا هو الحقل المطلوب
...
with tab_admin:
    ...
    if not st.session_state.logged_in:
        ...
        st.write("##")
        if l_col3.button(t[lang]["login_btn"]):
    ...
    if st.session_state.logged_in:
        ...
        # عرض الجدول مع التأكد من وجود عمود IssueDesc
        ...
        if all_ids:
            sel_id = st.selectbox("Select Ticket ID", all_ids)
            idx = df[df['ID'] == sel_id].index[0]
            
            # إظهار وصف المشكلة للطلب المختار بشكل بارز للمسؤول
            st.info(f"**وصف المشكلة للطلب {sel_id}:** \n\n {df.at[idx, 'IssueDesc']}")
            
            col_s, col_r = st.columns(2)
            new_stat = col_s.selectbox("الحالة", t[lang]["status_options"])
            new_rep = col_r.text_area("الرد", value=df.at[idx, 'Reply'])
            
            if st.button("تحديث الطلب"):
